...
const EmployeeSalary = () => {
    ...
    if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse text-[#161E54]">SYNCING COMPENSATION MATRIX...</div>;
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700 font-inter pb-20">
            {/* Command Header */}
            <div className="bg-[#161E54] p-12 rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
                ...
                <div className="lg:col-span-2 space-y-10">
                    <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden min-h-[400px]">
                        <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                            <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.4em] italic leading-none border-l-4 border-[#161E54] pl-6">Historical Ledger</h3>
                            ...
                            {payrolls.map(p => (
                                <div key={p._id} className="p-10 flex items-center justify-between group hover:bg-slate-50/50 transition-all">
                                    <div className="flex items-center gap-6">
                                        <div className="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black shadow-lg group-hover:scale-110 transition-transform">
                                            <Calendar size={20} />
                                        </div>
                                        <div>
                ...
                <div className="space-y-8">
                    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-10">
                            <div className="p-3 bg-indigo-50 text-[#161E54] rounded-2xl shadow-sm"><IndianRupee size={24} /></div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                                ...
                                <div className="text-right">
                                    <span className="text-3xl font-black text-[#161E54] tracking-tighter italic leading-none">₹{latest?.netSalary?.toLocaleString() || '0'}</span>
                                    <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mt-1 italic">Certified Fiscal Output</p>
                                </div>
                            </div>
                        </div>
                        <button className="w-full mt-12 py-6 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.4em] italic shadow-2xl shadow-slate-900/30 hover:scale-[1.02] active:scale-95 transition-all">Generate Digital Payslip</button>
                    </div>
                </div>
            </div>
        </div>
